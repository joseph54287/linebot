...
def quote_event(event_type="postback", user_id=None, data=""):
    event = {
        "type": event_type,
        "source": {
            "type": "user",
            "userId": user_id or main.QUOTE_DIRECTOR_USER_ID,
        },
    }
...
def test_quote_scheme_postback_is_forwarded():
    assert main.is_quote_event(
        quote_event(data="action=scheme&invitation=abc&scheme=A")
    )


def test_quote_confirmation_and_full_draft_are_forwarded():
    assert main.is_quote_event(quote_event("message", data="確認"))
    assert main.is_quote_event(quote_event("message", data="主旨：Re: 合作邀約"))


def test_expense_and_other_users_are_not_forwarded():
    assert not main.is_quote_event(quote_event(data="expense:confirm"))
    assert not main.is_quote_event(
        quote_event(user_id="U-other", data="action=scheme&invitation=abc&scheme=A")
    )
