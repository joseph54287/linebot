...
QUOTE_DIRECTOR_USER_ID = os.environ.get(
    "QUOTE_DIRECTOR_USER_ID",
...
def is_quote_event(event: dict[str, Any]) -> bool:
    ...
    if (
        ...
        or source.get("userId") != QUOTE_DIRECTOR_USER_ID
    ):
        return False

    if event.get("type") == "postback":
        data = event.get("postback", {}).get("data", "")
        return data.startswith("action=scheme&")
...
def forward_quote_webhook(body: bytes, signature: str) -> None:
    """保留 LINE 原始 body 與簽章，交由報價後端產生草稿或寄信。"""
    if not QUOTE_WEBHOOK_URL:
        raise requests.RequestException("quote webhook is not configured")
    response = requests.post(
        QUOTE_WEBHOOK_URL,
        data=body,
        headers={
...
async def webhook(request: Request):
    ...
    quote_events = [
        event for event in payload.get("events", []) if is_quote_event(event)
    ]
    if quote_events:
        try:
            ...
            return JSONResponse(
                {"error": "quote webhook forwarding failed"},
                status_code=502,
            )

    for event in payload.get("events", []):
        # 報價事件由專用後端使用同一個 reply token 處理，避免重複回覆。
                ...
                if action == "submit":
                    ...
                    if user_id == EXTERNAL_CASE_OWNER_USER_ID:
                        ...
                        reply_messages(reply_token, [
                            {"type": "text", "text": "已送出，現在是「待核准」。請在下方選擇「確認成立」或「待討論」。"},
                            external_case.approval_card(session["data"]),
                        ])
                        continue
